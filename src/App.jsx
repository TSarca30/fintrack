...
function LoginScreen({onLogin}){
  ...
  async function handleLogin(){
    ...
    try{
      ...
      if(user){onLogin(user);}else{setError("Username o password errati");}
  ...
  return(
    <div style={{background:PALETTE.bg,minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
      <div style={{width:"100%",maxWidth:380}}>
        ...
        <div style={{textAlign:"center",marginTop:20,fontSize:12,color:PALETTE.muted}}>Per accedere contatta l'amministratore</div>
      </div>
    </div>
  );
}
...
function ExportModal({transactions,accounts,categories,onClose}){
  ...
  function exportHTML(){
    ...
    <table><tr><th>Categoria</th><th>Totale</th></tr>
...
function TricountTab({accounts,categories,transactions,setTransactions,setAccounts,currentUser,allUsers}){
  ...
  // Carica gruppi e spese da Google Sheets
  async function loadTricount(){
    setLoadingTC(true);
    try{
      const [grps,exps]=await Promise.all([apiRead("tricount_groups"),apiRead("tricount_expenses")]);
      const allGroups=Array.isArray(grps)?grps:[];
      const allExpenses=Array.isArray(exps)?exps:[];
      // Mostra solo i gruppi dove l'utente è membro
      const myGroups=allGroups.filter(g=>{
        const members=String(g.members||"").split(",").map(m=>m.trim());
        return members.includes(currentUser.name);
      });
      setGroups(myGroups);
  ...
  async function createGroup(){
    ...
    const id=Date.now();
    const data={id,name:form.name,members:allMembers.join(","),createdBy:currentUser.name};
    await apiPost({action:"write",sheet:"tricount_groups",data});
    await loadTricount();
    setSaving(false);setModal(null);setActiveGroup(id);
  }

  async function deleteGroup(gid){
    if(!confirm("Eliminare questo gruppo?"))return;
    await apiPost({action:"delete",sheet:"tricount_groups",id:gid});
    // Elimina anche le spese del gruppo
    const groupExpenses=expenses.filter(e=>String(e.groupId)===String(gid));
    for(const exp of groupExpenses){
      await apiPost({action:"delete",sheet:"tricount_expenses",id:exp.id});
    }
    await loadTricount();
  ...
  function getSplits(g){
    ...
    if(form.splitType==="equal"){
      const each=(Number(form.amount)||0)/members.length;
      const s={};members.forEach(m=>s[m]=each.toFixed(2));return s;
    }
    return form.splits||{};
  }
  ...
  async function addExpense(){
    ...
    const id=Date.now();
    const expData={
      id,groupId:activeGroup,desc:form.desc,amount:Number(form.amount),
      paidBy:form.paidBy,splitType:form.splitType,
      splits:JSON.stringify(splits),catId:form.catId,date:form.date
    };
    await apiPost({action:"write",sheet:"tricount_expenses",data:expData});

    // Se pagato da me → scala dal mio conto personale
    if(form.paidBy===currentUser.name&&form.accountId){
      const cat=categories.find(c=>String(c.id)===String(form.catId));
      const txData={
        id:id+1,date:form.date,
  ...
  function getGroupExpenses(groupId){
    return expenses.filter(e=>String(e.groupId)===String(groupId)).map(e=>{
      let splits={};
      try{splits=JSON.parse(e.splits||"{}");}catch{splits={};}
      return{...e,splits,amount:Number(e.amount)};
    });
  }
    ...
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div>
          ...
          <div style={{fontSize:12,color:PALETTE.muted,marginTop:2}}>Condivise in tempo reale con tutti gli utenti</div>
        </div>
        <div style={{display:"flex",gap:8}}>
          <button className="btn-ghost" onClick={loadTricount} style={{padding:"6px 12px",fontSize:12}}>↻</button>
                ...
                <div key={g.id} className="card" style={{cursor:"pointer",borderLeft:`3px solid ${myNet>=0?PALETTE.green:PALETTE.red}`}} onClick={()=>setActiveGroup(g.id)}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                    <div>
                      ...
                      <div style={{fontSize:12,color:PALETTE.muted,marginTop:4}}>{expCount} spese</div>
                    </div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:16}}>
          ...
          <div className="card">
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
              <div style={{fontSize:14,fontWeight:600}}>Spese del gruppo ({getGroupExpenses(activeGroup).length})</div>
              <button className="btn" onClick={openAddExpense}>+ Aggiungi Spesa</button>
            </div>
            {getGroupExpenses(activeGroup).length===0?(
              ...
              {form.splitType==="equal"&&form.amount&&(
                <div style={{background:PALETTE.surface,borderRadius:12,padding:12}}>
                  <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                    {currentGroupMembers.map(m=>(
                      <span key={m} style={{fontSize:12,background:PALETTE.card,border:`1px solid ${PALETTE.border}`,borderRadius:8,padding:"4px 10px"}}>{m}: {fmt((Number(form.amount)||0)/currentGroupMembers.length)}</span>
                    ))}
                  </div>
                </div>
              )}
...
export default function App(){
  const [currentUser,setCurrentUser]=useState(()=>{try{return JSON.parse(sessionStorage.getItem("fintrack_user")||"null");}catch{return null;}});
  ...
  function handleLogin(user){sessionStorage.setItem("fintrack_user",JSON.stringify(user));setCurrentUser(user);}
  function handleLogout(){sessionStorage.removeItem("fintrack_user");setCurrentUser(null);setAccounts([]);setTransactions([]);setCategories([]);setBudgets([]);setGoals([]);}
  ...
  function getBudgetSpent(b){return monthlyTx.filter(t=>t.type==="expense"&&String(t.catId)===String(b.catId)&&t.subcat===b.subcat).reduce((s,t)=>s+Math.abs(Number(t.amount)),0);}
